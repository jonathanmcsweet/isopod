...
> **Under a Tier 3 microVM, isopod drops the `/sys` directory masks but keeps the `/proc` file masks.** The guest has its own kernel and only virtual devices (synthetic DMI, a virtio NIC/PCI bus), so the `/sys` device tree there describes the guest and masking it would protect nothing. The `/proc` file masks still matter: crun's krun handler hands the guest the **container's** rootfs over virtio-fs (`krun_add_virtiofs2(ctx, tag, "/")`), and the container's own `/proc` travels inside it as a second mount underneath the guest's procfs. Root in the box can unmount the guest's `/proc` and read the container's, where `/proc/cmdline` is still the **host** kernel's boot line (LUKS volume UUID, ostree deployment hash) and `/proc/config.gz` is the **host** kernel's build config. The VM is still the boundary for host *memory and kernel*; it is not a boundary for host *identity* leaked through those files.
