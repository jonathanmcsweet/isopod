...
- **microVM by default** — `isopod create` runs the box in a per-box guest kernel behind a KVM boundary (crun-vm/Kata) when a virtio-net microVM runtime and `/dev/kvm` are available. If not, it falls back to gVisor (`runsc`), then a plain container, with a warning. Pass **`--container`** to force a plain shared-kernel container. (`krun` is a microVM too, but its libkrun **TSI** networking stalls bulk data over an SSH port-forward into the box — the path VSCodium/Cursor/JetBrains use to reach their remote server — so `isopod code` can't connect and krun is **not** auto-selected. Its SSH exec path is fast, so it stays available via `--runtime krun` / `ISOPOD_RUNTIME=krun` for `isopod shell`/copy/export.)
