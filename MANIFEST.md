...
this tree. The two structural rules that matter:
  ...
  1. `lib/` MUST sit beside the `aibox` script (aibox streams
  ...
  2. `test/libs/` holds the vendored bats framework — keep it intact to run
     the test suite, but it is NOT needed to USE aibox. The installer excludes
     `test/` from installs.

```
aibox-project/
├── aibox                     # the CLI (bash)            [executable]
...
│   └── compose.yaml          # reference Compose form of the same masks (not run by aibox)
└── test/
...
```sh
chmod +x aibox install.sh lib/apply_color.py \
...
```sh
git update-index --chmod=+x aibox install.sh lib/apply_color.py \
         test/run.sh test/interactive_test.py
```
