...
> **Canonical file.** This is `AGENTS.md`, the cross-agent standard filename.
> The agent-specific filenames — `CLAUDE.md` (Claude Code), `.cursorrules`
> (Cursor), `.windsurfrules` (Windsurf), and `.github/copilot-instructions.md`
> (Copilot) — are symlinks to this file so every agent reads the same rules.
> Edit this file; the symlinks follow.
