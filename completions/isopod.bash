...
_isopod() {
  ...
  local cmds="create list info code shell claude-code root-shell upgrade start stop config reconfigure migrate export fetch remap copy-in rm egress host-port account secret doctor help version"
  ...
  case "$sub" in
    info | code | shell | claude-code | root-shell | upgrade | start | stop | config | reconfigure | export | fetch | remap | copy-in | rm)
