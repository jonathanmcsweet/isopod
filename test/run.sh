...
if command -v shellcheck >/dev/null 2>&1; then
  shellcheck -S warning isopod share/isopod-entrypoint lib/find_box_repo.sh install.sh test/run.sh test/packaging.sh test/brew-formula.sh verify-host-isolation.sh
...
SHFMT_FILES="isopod share/isopod-entrypoint lib/find_box_repo.sh install.sh verify-host-isolation.sh test/run.sh test/packaging.sh test/brew-formula.sh test/helper.bash completions/isopod.bash"
